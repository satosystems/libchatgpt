...
class cb_bucket {
public:
    explicit cb_bucket(chatgpt_callback cb) : cb(cb) {};
    chatgpt_callback cb;
    std::string id;
};
...
static size_t write_callback(char *ptr, size_t size, size_t nmemb, void *clientp) {
    ...
    for (const std::string& line : lines) {
        if (line.starts_with("data: {")) {
            ...
            if (bucket->id.empty()) {
                bucket->id = j["id"];
                bucket->cb(chatgpt_callback_kind_id, bucket->id.c_str());
            }
            ...
            if (!choice.is_null()) {
                ...
                if (!content.is_null()) {
                    ...
                    bucket->cb(chatgpt_callback_kind_content, content_s.c_str());
                }
            }
        } else if (line.starts_with("data: [DONE]")) {
            bucket->cb(chatgpt_callback_kind_stop, nullptr);
        }
    }
...
void chatgpt_completions(const char *api_key, int length, chatgpt_message messages[], chatgpt_callback cb) {
    ...
    if (curl == nullptr) {
        cb(chatgpt_callback_kind_error, "curl_easy_init() failed.");
        return;
    }
    ...
    if (rc != CURLE_OK) {
        cb(chatgpt_callback_kind_error, "curl_easy_perform() failed.");
    }
}
